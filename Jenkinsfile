...
//                            openshift.patch("deployment/${env.mainAppName}", "'{\"spec\":{\"template\":{\"metadata\":{\"annotations\":{\"sidecar.istio.io/inject\":\"true\"}}}}}'")
                            ...
                            echo "Expose Service with Secure Route"
                            openshift.create("route", "edge", env.mainAppName, "--service=${env.mainAppName} --port=8080")
                            echo "Add label for route to internal"
                            openshift.selector("route", env.mainAppName).label([ 'exposure':'internal' ], "--overwrite")
...
//                            echo "Add environment variable for JDBC client from secret"
//                            openshift.set('env', "deployment/${env.mainAppName}", '--from=secret/jdbc-client', '--prefix=OCP_SPRING_JDBC_')
//                            echo "Add volume for truststore from secret"
//                            openshift.set('volume', "deployment/${env.mainAppName}", "--add", "--secret-name=${env.mainAppName}-truststore", "--name=${env.mainAppName}-truststore-volume", "--mount-path=/truststore")
...
//                            echo "Create istio gateway from template"
//                            def models = openshift.process( "openshift//istio-gateway-api", "-p", "NAMESPACE=${env.OPENSHIFT_PROJECT}", "-p", "APP_NAME=${env.mainAppName}" )
//                            openshift.create( models )
//                            echo "Patch istio-ingressgateway route to be tls edge termination"
//                            def istioRoute = openshift.selector( "route -n istio-system", [ "maistra.io/gateway-name": "${env.mainAppName}", "maistra.io/gateway-namespace": "${env.OPENSHIFT_PROJECT}" ] ).object().metadata.name
//                            openshift.patch( "route/${istioRoute}", "'{\"spec\":{\"tls\":{\"termination\":\"edge\"}}}' -n istio-system")
